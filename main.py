...
def CheckUserInfo(self):
    ...
    check_keys = ["user_token", "user_secret", "screen_name", "picture"]
    #Cookieのキーをチェックし、存在の有無をチェック
    if all(cookie.has_key(key) for key in check_keys):
      return True
    else :
      return False
...
def ReplaceLink(str) :
    #URL
    str = re.sub("(https://[\w|/|\.|%|&|\?|=|\-|#]+)", r'<a href="\1" target=\"_blank\">\1</a>', str)
    #ユーザー名
...
class MainHandler(webapp.RequestHandler):
    ...
    def get(self, mode=""):
        ...
        if mode == "delete":
            ...
            client.make_request('https://api.twitter.com/1.1/statuses/destroy/' + status_id + '.json',
    ...
    def post(self, mode=""):
        ...
        if mode == "tweet":
            ...
            self.post_core(client, cookie["user_token"], cookie["user_secret"], param)

            #つぶやき追加後にタイムラインを改めて表示
            return self.redirect("/")

    def post_core(self, client, token, secret, params):
